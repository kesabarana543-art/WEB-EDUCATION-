...
async function openApp(appName: string): Promise<void> {
    ...
    if (iconElement) {
        ...
        if(span) title = span.textContent || appName;
         ...
         switch(appName) {
            ...
            case 'webEdu': iconSrc = 'https://win98icons.alexmeub.com/icons/png/help_book_computer-0.png'; title = 'Web Education'; break;
         }
    }
    ...
    else if (appName === 'webEdu') {
        initWebEdu(windowElement);
    }
}
...
// --- Web Education AI Tutor Logic ---
function initWebEdu(windowElement: HTMLDivElement): void {
    const topicsList = windowElement.querySelector('#webedu-topics') as HTMLUListElement;
    const contentArea = windowElement.querySelector('#webedu-content') as HTMLDivElement;
    if (!topicsList || !contentArea) return;
        ...
        if (target.tagName.toLowerCase() === 'li') {
            ...
            if (topic) {
                ...
                Array.from(topicsList.children).forEach(li => {
                    (li as HTMLElement).style.background = 'transparent';
                    (li as HTMLElement).style.color = 'blue';
                });
                target.style.background = '#000080';
                target.style.color = 'white';

                contentArea.innerHTML = `<p>Loading lesson on <strong>${topic}</strong>...</p><p style="color:#808080"><em>Dialing out to AI server...</em></p>`;
                
                try {
                    if (!geminiInstance) {
                        if (!await initializeGeminiIfNeeded('initWebEdu')) {
                            contentArea.innerHTML = "<p style='color:red;'>Failed to initialize AI Tutor.</p>";
                            return;
                        }
                    }
                    
                    const prompt = `You are a computer science teacher in the year 1995. Write a brief, engaging, encyclopedic explanation about "${topic}" (approx 150 words). Format the output in simple HTML using <h3> and <p> tags. Keep references strictly relevant to 1990s technology (e.g., dial-up, NCSA Mosaic, Netscape, HTML 2.0).`;
                     // @ts-ignore
                    const result = await geminiInstance.models.generateContent({
                    ...
                    const lessonHTML = result?.text?.trim() || "<p>An error occurred generating the lesson.</p>";
                    // clean up markdown bounds just in case
                    contentArea.innerHTML = lessonHTML.replace(/```html|```/gi, '');
                } catch (err: any) {
                    contentArea.innerHTML = `<p style="color:red;">Error: ${err.message}</p>`;
                }
            }
        }
    });
